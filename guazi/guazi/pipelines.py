...
class ExcelPipeline(object):
    def __init__(self):
        ...
        self.ws.append(['品牌名称', '品牌ID', '车系名称', '车系ID', '车型名称', '车型ID', '年款', '变速箱类型', '座位数', '抓取时间'])

    def process_item(self, item, spider):
        line = [item['brandname'], item['brand_id'], item['familyname'], item['family_id'], item['vehicle'],
                item['vehicle_id'], item['years'], item['transmission'], item['seats'], item['grab_time']]
        self.ws.append(line)
        self.wb.save('瓜子车型库.xlsx')
        return item
...
class GuaziPipeline:
    ...
    def __init__(self, settings):
        ...
        filename = str(pathlib.Path.cwd()) + '/blm/' + website + '/' + settings[
            'MONGODB_COLLECTION'] + '.blm'
        dirname = str(pathlib.Path.cwd()) + '/blm/' + website
    ...
    def process_item(self, item, spider):
        ...
        # mysql有要去重字段status的爬虫名字写进去
            ...
            if returndf:
                ...
                df.to_sql(name=self.collection, con=self.conn, if_exists="append", index=False)
